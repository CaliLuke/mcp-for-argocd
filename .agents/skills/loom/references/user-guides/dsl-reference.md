...
- HTTP Guide: WebSocket Integration — HTTP-specific streaming implementation
- HTTP Guide: Server-Sent Events — SSE for one-way server streaming
- gRPC Guide: Streaming Patterns — gRPC streaming implementation
...
Note: `Files` is HTTP-only. For detailed patterns including template integration, see HTTP Guide: Static Content.

## Error Handling (Design Level)

Errors in Goa are defined at the design level and automatically mapped to transport-specific responses. This section covers the DSL for defining errors. For transport mapping details, see Error Handling Guide.
...
- HTTP Guide: Error Responses — HTTP status code mapping
- gRPC Guide: Error Handling — gRPC status code mapping
