...
Wire rate-limited clients into the loom-mcp runtime:
...
Temporal provides durable execution for your loom-mcp agents. Agent runs become Temporal workflows with event-sourced history. Tool calls become activities with configurable retries. Every state transition is persisted. A restarted worker replays history and resumes exactly where it left off.
...
loom-mcp abstracts the execution backend behind the Engine interface. Swap engines without changing agent code:
...
This section shows how to stream agent events to UIs in real-time using loom-mcp’s streaming infrastructure.


Overview
--------

loom-mcp publishes session-owned streams of typed events that can be delivered to UIs via:
